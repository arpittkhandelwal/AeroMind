...
export function initVanillaScene(container, opts) {
  var onStateChange = (opts && opts.onStateChange) || null;
...
// Make sure the canvas element fills the container div
...
/* ============================================================
   6. UI STUBS (React handles UI; only 3D scene logic kept)
   ============================================================ */
var $ = function(id){ return {
  textContent:"", innerHTML:"", className:"",
  style:{background:"",color:"",left:"",top:"",width:"",borderLeftColor:""},
  classList:{add:function(){},remove:function(){},toggle:function(){}},
  children:[], querySelector:function(){ return {textContent:"",style:{},className:"",innerHTML:""}; },
...
var teleRows = {};
TELE_KEYS.forEach(function(k){ teleRows[k] = $('dummy'); });
var subRows = {};
Object.keys(PARTS).forEach(function(id){ subRows[id] = $('dummy'); });

function log(msg, cls){ console.log('[UAV Twin]', msg); }

function select(id){ S.selected = id; }
function closeInspector(){ S.selected = null; }
function renderInspector(){ /* noop – React renders this */ }

function setStatus(text, sub){
  S.status = text;
  if(onStateChange) onStateChange(text, sub || '');
}

function renderWhy(){ /* noop – React renders this */ }

/* ---- hotspot spot (invisible div stub) ---- */
var spot = { classList:{add:function(){},remove:function(){}}, style:{} };
var wv = new THREE.Vector3();
function updateSpot(){
  if(!S.fault || S.recovering || S.destroyed){ return; }
...
function destroy(){
  ...
  $('rul').classList.remove('on','hot'); // noop via stub
  spot.classList.add('hidden'); // noop via stub
  ...
  // loss overlay / DOM handled by React
  renderInspector();
}
  ...
  if(S.fault && !S.destroyed){
    if(S.recovering){
      ...
      if(frac <= 0 ){ destroy(); }
        ...
        // rul hot highlight handled by React
...
function refreshUI(){
  // Telemetry and UI updates handled by React via onStateChange / props
  if(S.running) S.endurance = Math.max(0, S.endurance - 0.2*3);

  if(S.fault && !S.recovering && !S.destroyed){
    var r = Math.ceil(S.rul);
    // rul-clock display handled by React
  }

  // renderWhy / renderInspector — handled by React UI
}
...
/* ============================================================
   ...
   ============================================================ */
function resize(){
  var w = wrap.clientWidth || wrap.offsetWidth || 800;
  var h = wrap.clientHeight || wrap.offsetHeight || 600;
  if(w === 0 || h === 0) { w = 800; h = 600; }
  renderer.setSize(w, h, false);
  camera.aspect = w / h; camera.updateProjectionMatrix();
}

// Defer first resize so the container has layout dimensions
requestAnimationFrame(function(){
  resize();
  setStatus('STANDBY','engine off');
  log('Digital twin linked \u00b7 21 sensor channels streaming');
});
var _rafId = requestAnimationFrame(tick);


  return {
    resize: resize,
    setRunning: function(v){ S.running = v; },
    cleanup: function(){ cancelAnimationFrame(_rafId); renderer.dispose(); }
  };
}
