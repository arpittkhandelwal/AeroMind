...
function XWing({ xPos, chord, span, sweep, mat, hasDecal, onPointerDown, sel }) {
  const hlt = '#FF9933';
  const topMat = sel ? new THREE.MeshStandardMaterial({ color: hlt, roughness:0.9 }) : mat.top;
...
function UAVBody({ mat, telemetry, healthScore, selectedPart, onSelectPart }) {
  ...
  const nacelleMat = useMemo(() => new THREE.MeshStandardMaterial({
    color:0x5f666d, roughness:0.7, metalness:0.2,
    emissive: engineEmissive,
    emissiveIntensity: telemetry?.cht_c > 240 ? 1.0 : telemetry?.cht_c > 200 ? 0.4 : 0.05,
  }), [engineEmissive, telemetry?.cht_c]);

  const getBodyMat = (part) => isSel(part)
  ...
  return (
    <group>
      ...
      <mesh castShadow rotation={[0,0,Math.PI/2]} position={[2.0,0,0]} onPointerDown={e=>handle(e,'avionics')}>
        ...
        {isSel('avionics') && (
          <Html center distanceFactor={10} position={[0,1.4,0]}>
            <div className="tooltip3d"><div className="title">Avionics Bay</div>
              <div>Battery: {telemetry?.battery_voltage_v?.toFixed(1)??'—'} V</div>
      ...
      <mesh castShadow rotation={[0,0,Math.PI/2]} position={[-1.9,0,0]} onPointerDown={e=>handle(e,'fuel')}>
        ...
        {isSel('fuel') && (
          <Html center distanceFactor={10} position={[0,1.2,0]}>
            <div className="tooltip3d"><div className="title">Fuel Bay</div>
              <div>Fuel Flow: {telemetry?.fuel_flow_lph?.toFixed(2)??'—'} L/h</div></div>
          </Html>
        )}
      </mesh>
      {/* Aft fuselage */}
      ...
      <mesh castShadow position={[5.35,-0.08,0]} onPointerDown={e=>handle(e,'turret')}>
        ...
        {isSel('turret') && (
          <Html center distanceFactor={10} position={[0,1,0]}>
            <div className="tooltip3d"><div className="title">Nose Sensor Turret</div>
              <div>Vibration: {telemetry?.vibration_g?.toFixed(3)??'—'} g</div>
              <div>Alt: {telemetry?.altitude_m?.toFixed(0)??'—'} m</div></div>
          </Html>
        )}
      </mesh>
      ...
      <XWing xPos={1.3} chord={2.4} span={3.5} sweep={0.9} mat={mat} hasDecal
        onPointerDown={e=>handle(e,'wingFwd')} sel={isSel('wingFwd')} />
      {isSel('wingFwd') && (
        <Html center distanceFactor={10} position={[1.3,4.5,0]}>
          <div className="tooltip3d"><div className="title">Forward X-Wing</div>
            <div>Vibration: {telemetry?.vibration_g?.toFixed(3)??'—'} g</div></div>
        </Html>
      )}

      {/* Aft X-wing */}
      <XWing xPos={-3.9} chord={1.9} span={2.7} sweep={0.7} mat={mat} hasDecal={false}
        onPointerDown={e=>handle(e,'wingAft')} sel={isSel('wingAft')} />
      {isSel('wingAft') && (
        <Html center distanceFactor={10} position={[-3.9,3.5,0]}>
          <div className="tooltip3d"><div className="title">Aft X-Wing</div>
            <div>Throttle: {telemetry?.throttle_pct?.toFixed(1)??'—'}%</div></div>
        </Html>
      )}
      ...
      <mesh castShadow rotation={[0,0,Math.PI/2]} position={[-6.5,0,0]} onPointerDown={e=>handle(e,'engine')}>
        ...
        {isSel('engine') && (
          <Html center distanceFactor={10} position={[0,1.5,0]}>
            <div className="tooltip3d engine"><div className="title">Engine &amp; Propeller</div>
              <div>RPM: {telemetry?.rpm?.toFixed(0)??'—'}</div>
              <div style={{color:telemetry?.cht_c>240?'#ef4444':'inherit'}}>CHT: {telemetry?.cht_c?.toFixed(1)??'—'}°C</div>
              <div>EGT: {telemetry?.egt_c?.toFixed(1)??'—'}°C</div>
              <div>Oil P: {telemetry?.oil_pressure_kpa?.toFixed(0)??'—'} kPa</div>
            </div>
          </Html>
        )}
      </mesh>
...
export default function DroneModel({ telemetry, healthScore=100, selectedPart, onSelectPart }) {
  ...
  useFrame((state) => {
    ...
    const vib = telemetry?.vibration_g || 0;
    groupRef.current.rotation.y = t * 0.18;
    groupRef.current.position.y = Math.sin(t*0.9)*0.12;
    groupRef.current.rotation.z = Math.sin(t*0.45)*0.03;
    const shake = vib * 2;
    groupRef.current.position.x = Math.sin(t*28)*shake*0.018;
    groupRef.current.position.z = Math.cos(t*32)*shake*0.014;
  });
